...
- **2026-07-21 (follow-up 3) — full E2E live test + verify determinism fix + Google Places DOWN:** ran a complete live E2E (auth, pages, dashboard, all lead filters, Gemini analysis, activities, discovery sweep, refresh, delete/contacted, quota metering, production smoke) — **all green** except two: (1) **Verify was non-deterministic** on Tavily `basic`/10 results (a lead's has-site verdict could flip run-to-run) → fixed by `search_depth:"advanced"` + `max_results:15`; direct test now 5/5 runs stable. (2) **Google Places returns 403 PERMISSION_DENIED.** Two separate causes: (i) Cloud **billing was suspended** for a missing payment method (RBI e-mandate) — Sumit **re-enabled billing via UPI autopay/e-mandate on 2026-07-21**; (ii) but Places STILL 403s with the same bare message → the remaining blocker is an **API-key restriction and/or Places API (New) not enabled** on the project (a console fix, NOT billing). **Guard against charges (autopay now live):** set a low daily **Requests/day quota cap** on the Places SKUs in the Cloud console (APIs & Services → Places API (New) → Quotas) — the hard no-bill guarantee; budget alerts only notify. App-side Quota Guardian already hard-stops at 90% of `google_places` (1,000/mo). **Gemini verified independent** (AI Studio free-tier endpoint, still HTTP 200) — AI analysis unaffected. App runs fine on card-free OSM+TomTom meanwhile; production confirmed live (shared Neon DB, `TAVILY_API_KEY` set on Vercel).
